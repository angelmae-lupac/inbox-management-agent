...
export async function analyzeMessageWithGemini(message: {
  ...
  try {
    ...
    const category = normalizeCategory(parsed.category ?? "Routine");
    ...
    return {
      ...
      needsHumanReview,
      extracted: {
        orderNumber: parsed.extracted?.orderNumber ?? undefined,
        requestedDate: parsed.extracted?.requestedDate ?? undefined,
        customerName: parsed.extracted?.customerName ?? undefined,
        issueType: parsed.extracted?.issueType ?? undefined,
      },
