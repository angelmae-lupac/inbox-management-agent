...
export function InboxDashboard({
  ...
  const [replyBody, setReplyBody] = useState(initialMessages[0]?.draftReply ?? "");
  const [sendState, setSendState] = useState({ loading: false, error: "" });
  const [gmailState, setGmailState] = useState({ loading: false, error: "" });
  ...
  const analyzeMessage = useCallback(async (message: InboxMessage) => {
    if (demoMode) return;
    if (analyzingIds.current.has(message.id)) return;
    analyzingIds.current.add(message.id);
    ...
    try {
      ...
      analyzingIds.current.delete(message.id);
    }
  }, [addActivity, demoMode]);

  useEffect(() => {
    const message = selectedMessageRef.current;
    if (message && !demoMode) void analyzeMessage(message);
  }, [analyzeMessage, demoMode, selectedMessage?.id]);

  const sendReply = async () => {
    if (demoMode || !selectedMessage || !replyBody.trim() || sendState.loading) return;

    setSendState({ loading: true, error: "" });

    try {
      const response = await fetch(`/api/messages/${selectedMessage.id}/send`, {
        method: "POST",
      ...
      setMessages((current) =>
        current.map((message) =>
          message.id === selectedMessage.id
            ? { ...message, status: "Handled", deliveryStatus: "Sent" }
            : message,
        ),
      );
      ...
      addActivity({
        messageId: selectedMessage.id,
        title: "Reply sent",
        detail: `Reply accepted by the email provider for ${selectedMessage.email}.`,
        tone: "success",
      });
    } catch (error) {
      setSendState({
        loading: false,
        error: error instanceof Error ? error.message : "Unable to send this reply.",
      });
    }
  };
  ...
  const importGmail = async () => {
    if (demoMode) return;
    setGmailState({ loading: true, error: "" });
    try {
      const response = await fetch("/api/gmail/import", { method: "POST" });
      const data = await response.json();
      if (!response.ok || !data?.success) throw new Error(data?.error || "Unable to import Gmail.");
      window.location.reload();
    } catch (error) {
      setGmailState({ loading: false, error: error instanceof Error ? error.message : "Unable to import Gmail." });
    }
  };
          ...
          <div className="border-b border-slate-200 bg-slate-950 px-6 py-5 text-white">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                {demoMode ? (
                  <div className="rounded-full border border-amber-300/30 bg-amber-400/15 px-4 py-2 text-sm font-semibold text-amber-100">
                    Portfolio demo
                  </div>
                ) : (
                  <>
                    ...
                    <button
                      ...
                      className="rounded-full border border-emerald-300/30 bg-emerald-400/15 px-4 py-2 text-sm font-semibold text-emerald-100 transition hover:bg-emerald-400/25 disabled:opacity-50"
                    >
                      {gmailState.loading ? "Importing..." : "Import Gmail"}
                    </button>
                  </>
                )}
              </div>
              ...
              {visibleMessages.map((message) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    setReplyBody(message.draftReply);
                    setSendState({ loading: false, error: "" });
                  }}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
                      ...
                      <textarea
                        ...
                        rows={6}
                        disabled={demoMode || selectedMessage.requiresHumanReview || selectedMessage.deliveryStatus === "Sent"}
                        className="w-full resize-y rounded-xl border border-blue-200 bg-white p-3 text-sm leading-6 text-slate-700 outline-none ring-blue-300 transition focus:ring-2 disabled:cursor-not-allowed disabled:bg-slate-100"
                        ...
                        <span className="text-xs text-slate-500">
                          {selectedMessage.requiresHumanReview
                            ...
                            : selectedMessage.deliveryStatus === "Sent"
                              ...
                              : "Review the draft before sending. " + selectedMessage.email}
                        </span>
                        <button
                          type="button"
                          onClick={() => void sendReply()}
                          disabled={demoMode || selectedMessage.requiresHumanReview || selectedMessage.deliveryStatus === "Sent" || sendState.loading || !replyBody.trim()}
                          className="rounded-full bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:bg-slate-400"
                        >
                          {demoMode ? "Demo send disabled" : sendState.loading ? "Sending..." : selectedMessage.deliveryStatus === "Failed" ? "Retry send" : "Approve and send"}
                        </button>
                      </div>
                      {sendState.error && <p className="mt-3 text-sm font-medium text-red-700">{sendState.error}</p>}
                    </div>
                ...
                <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Delivery state is updated from the email provider.</p>
                    </div>
